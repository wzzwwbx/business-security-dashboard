...
const pages: Record<string, DashboardPage> = {
  overview: {
    code: 'overview',
    name: '态势总览',
    title: '业 务 安 全 态 势 总 览',
    subtitle: '后台服务中心机房 · 业务安全态势系统',
    location: '后台服务中心机房',
    ...
    summaryMetrics: [
      ...
      { label: 'USB Key 合规率', value: '96.8', unit: '%', trend: '+1.1%', status: 'success', description: '认证成功率持续提升' },
      ...
      { label: 'AI 检测命中', value: '23', unit: '条', trend: '+5 条', status: 'info', description: 'KNN + LSTM 融合分析' }
    ],
    widgets: [
      widget('terminal-access', '终端接入状态', 'statusGrid', 4, 320, ['终端', '零信任'], {
        items: [
          statusItem('境外 PAD-01', '4G 国际互联网接入 · 最近 3 分钟活跃', 'success', 98),
          statusItem('境外 PAD-02', '卫星链路抖动 12ms · 建议持续观察', 'warning', 81),
      ...
      widget('usb-auth', 'USB Key 认证状态统计', 'pieChart', 4, 320, ['认证', '密码装备'], {
        option: pieChart([
          { name: '认证成功', value: 96 },
          { name: '待复核', value: 6 },
          { name: '故障', value: 2 }
      ...
      widget('overview-topology', '后台服务中心机房 · 系统拓扑', 'topology', 12, 440, ['拓扑', '主链路', '中枢'], {
        nodes: [
          { id: 'gateway', name: '接入网关', meta: '4G / 卫星双链路', status: 'success', x: 12, y: 50 },
          { id: 'collector', name: '态势采集服务', meta: '归一化接入 5 类系统', status: 'info', x: 30, y: 22 },
          { id: 'mail', name: '密信交换系统', meta: '电报 / 消息 / 数字信封', status: 'success', x: 24, y: 50 },
          { id: 'analysis', name: '智能分析中心', meta: 'KNN / LSTM 融合引擎', status: 'success', x: 50, y: 50 },
          { id: 'sign', name: '签阅流转系统', meta: 'OFD 批注 / 签章验证', status: 'warning', x: 76, y: 50 },
          { id: 'zt', name: '零信任策略中心', meta: '策略下发 + 风险评估', status: 'warning', x: 70, y: 22 },
          { id: 'defense', name: '动态防御系统', meta: '终端行为采集 / 安全事件', status: 'success', x: 30, y: 80 },
          { id: 'db', name: '数据存储集群', meta: 'MySQL + 时序归档', status: 'info', x: 50, y: 84 },
          { id: 'screen', name: '态势展示门户', meta: '大屏 / 值守门户 / 运维台', status: 'success', x: 88, y: 52 }
        ],
        lines: [
      ...
      widget('risk-trend', '今日零信任风险趋势（近12h）', 'lineChart', 4, 320, ['零信任', '趋势'], {
  ...
  terminal: {
    ...
    name: '终端态势',
    title: '终 端 主 题 态 势',
    subtitle: '移动电报终端 / 签批 PAD / USB Key 统一感知',
    ...
    summaryMetrics: [
      ...
      { label: 'USB Key 正常率', value: '95.6', unit: '%', trend: '+0.5%', status: 'info', description: '认证链路稳定' }
    ],
    widgets: [
      widget('asset-map', '资产位置地图 · 境外保障态势', 'nodeMap', 6, 320, ['节点', '资产'], {
        regions: [
          { name: '东南亚', count: 28, items: ['PAD-SG-01', 'PAD-SG-02', 'KEY-SG-07', 'GW-SG'] },
          { name: '西亚', count: 16, items: ['PAD-DJ-01', 'PAD-DJ-02', 'KEY-DJ-03'] },
          { name: '欧洲', count: 24, items: ['PAD-FR-01', 'PAD-DE-02', 'KEY-EU-11', 'GW-EU'] },
          { name: '非洲', count: 12, items: ['PAD-KE-02', 'KEY-KE-03', 'SAT-KE'] }
        ]
      }),
      widget('asset-roster', '资产清单 · 140 台覆盖 4 大洲 19 个节点', 'table', 6, 320, ['清单', '台账'], {
        ...
        rows: [
          { asset: 'PAD-SG-01', type: '签批 PAD', location: '新加坡', status: '正常', owner: '海外保障一组' },
          { asset: 'PAD-DJ-02', type: '移动终端', location: '迪拜', status: '处理中', owner: '海外保障二组' },
          { asset: 'KEY-EU-11', type: 'USB Key', location: '法兰克福', status: '正常', owner: '欧洲保障组' },
      ...
      widget('terminal-key', 'USB Key 认证状态', 'pieChart', 3, 320, ['认证', '密钥'], {
        option: pieChart([
          { name: '成功', value: 132 },
          { name: '待复核', value: 5 },
          { name: '失败', value: 3 }
  ...
  business: {
    ...
    widgets: [
      ...
      widget('biz-kpi', '业务处理 SLA 监测', 'statusGrid', 6, 320, ['SLA', '业务'], {
        items: [
      ...
      widget('biz-table', '重点业务样本', 'table', 6, 320, ['样本', '重点'], {
        ...
        rows: [
          ...
          { time: '17:58', type: '归档同步', owner: '平台组', status: '待确认', cost: '8 分钟' }
        ]
      })
    ]
  },
  security: {
    ...
    subtitle: '动态防御安全事件 / 零信任风险评估 / AI 异常行为检测',
    location: '后台服务中心机房 + 终端侧零信任组件',
    lastUpdated: '2026-07-06 19:30:00',
    dataMode: 'mock',
    summaryMetrics: [
      ...
      { label: 'AI 分析告警', value: '23', unit: '条', trend: '+5', status: 'info', description: '已进入事件研判队列' },
    ...
    widgets: [
      widget('risk-gauge', '零信任风险评估', 'gaugeChart', 3, 320, ['风险', '评估'], {
        option: gaugeChart()
      }),
      widget('security-timeline', '安全事件时间线', 'timeline', 5, 320, ['时间线', '事件'], {
        items: [
          { time: '19:12', title: '境外账号异常提权尝试', description: '零信任策略已执行高风险二次认证，账号行为已进入复核。', status: 'danger' },
          ...
          { time: '17:54', title: 'AI 模型命中异常行为序列', description: '命中用户深夜高频签收模式，已推送安全研判。', status: 'info' }
        ]
      }),
      widget('ai-alert', '异常行为告警（AI 分析）', 'alertList', 4, 320, ['AI', '告警'], {
        items: [
          ...
          { level: '提示', title: '模型参数更新完成', description: 'KNN 与 LSTM 参数热更新生效。', time: '18:02', status: 'info' }
        ]
      }),
      ...
      widget('crypto-equipment', '密码装备状态', 'statusGrid', 4, 320, ['密码装备', '状态'], {
        items: [
          ...
          statusItem('USB Key 管理服务', '2 个实例延迟偏高', 'warning', 84),
  ...
  ops: {
    ...
    summaryMetrics: [
      ...
      { label: '资源告警', value: '4', unit: '项', trend: '-1', status: 'warning', description: 'CPU 与链路告警为主' },
    ...
    widgets: [
      widget('ops-devices', '设备运行状态', 'statusGrid', 6, 320, ['设备', '运行'], {
        items: [
          statusItem('态势采集服务器', 'CPU 峰值 78%，建议扩容', 'warning', 78),
      ...
      widget('ops-policy', '策略下发记录', 'table', 6, 320, ['策略', '执行'], {
        ...
        rows: [
          { time: '19:02', policy: '外设禁用', target: 'PAD-DJ-02', status: '已执行', latency: '14 秒' },
          ...
          { time: '17:41', policy: '终端锁定 30 分钟', target: 'PAD-SG-03', status: '已执行', latency: '5 秒' }
        ]
      }),
      widget('ops-recommend', '处置策略推荐', 'recommendationList', 6, 320, ['推荐', '处置'], {
        items: [
          { priority: 'P1', title: '提升高风险用户认证级别', description: '针对 4 名高风险用户，建议立即切换到双因子 + 设备指纹复核策略。', action: '执行二次认证', target: '零信任策略中心' },
          { priority: 'P2', title: '扩容采集线程池', description: '态势采集服务器在晚高峰 CPU 超过 75%，建议增加 2 个采集工作线程。', action: '调整服务参数', target: '态势采集服务' },
      ...
      widget('ops-logs', '系统日志', 'alertList', 6, 320, ['日志', '审计'], {
        items: [
          { level: '提示', title: 'Syslog 接入成功', description: '密信系统最新日志批次已写入归档通道。', time: '19:14', status: 'success' },
          { level: '告警', title: '采集服务器 CPU 偏高', description: '采集峰值 78%，已生成扩容建议。', time: '18:59', status: 'warning' },
      ...
      widget('ops-topology', '运维资源拓扑', 'topology', 12, 340, ['机房', '资源拓扑'], {
        nodes: [
          ...
          { id: 'collector', name: '态势采集服务器', meta: '日志 / API / MQ 接入', status: 'warning', x: 48, y: 35 },
          { id: 'analysis', name: '智能分析服务器', meta: '规则引擎 + 模型推理', status: 'info', x: 64, y: 58 },
          { id: 'database', name: '数据库服务器', meta: 'MySQL 主库', status: 'success', x: 74, y: 25 },
