...
const regionRows = computed(() => demoSituationScenario.regions
  ...
  .map((region) => {
    ...
    if (activeTopic.value === 'traffic') {
      ...
      return { name, primary: `${load.toFixed(1)} Mbps`, secondary: `上行 ${region.uplinkMbps.toFixed(1)} / 下行 ${region.downlinkMbps.toFixed(1)} Mbps · ${region.trafficGb.toFixed(1)} GB`, tone: load >= 14 ? 'warning' : 'success' };
    }
    if (activeTopic.value === 'message') {
      return { name, primary: `${region.people.filter((person) => person.online).length}/${region.people.length} 登录`, secondary: `${region.people.reduce((sum, person) => sum + person.message.sentMessages + person.message.receivedMessages, 0)} 条消息`, tone: region.people.some((person) => person.suiteStatus === 'offline') ? 'danger' : 'success' };
    }
    return { name, primary: `${region.people.reduce((sum, person) => sum + person.signing.processed, 0)} 份已处理`, secondary: `${region.people.reduce((sum, person) => sum + person.signing.pending, 0)} 份待处理`, tone: region.people.reduce((sum, person) => sum + person.signing.exception, 0) > 0 ? 'warning' : 'success' };
  }));
...
<style scoped>
.demo-business { position: relative; height: calc(100vh - var(--topbar-height) - 22px); min-height: 640px; display: grid; grid-template-rows: auto auto minmax(0,1fr) 168px; gap: 10px; color: #eef5ff; background: radial-gradient(circle at 50% 42%, rgba(61, 137, 214, .1), transparent 48%); }
.topic-switch { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)) 170px; border: 1px solid #385171; background: #172337; }.topic-switch button { min-height: 52px; display: flex; align-items: center; justify-content: center; gap: 9px; border: 0; border-right: 1px solid #385171; color: #7e8a9e; background: transparent; cursor: pointer; }.topic-switch button:last-child { border-right: 0; }.topic-switch button.active { color: #dce6f5; background: #19243a; box-shadow: inset 0 -2px #6d9bea; }.topic-switch strong { font-size: 20px; white-space: nowrap; }.topic-switch span { font-size: 16px; white-space: nowrap; }.topic-switch .topic-back { flex: 0 0 170px; color: #7fb0ff; font-size: 16px; gap: 6px; white-space: nowrap; }.topic-switch .topic-back:hover { color: #cfe2ff; background: #182338; }
.metric-strip { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); border: 1px solid #385171; background: #172337; }.metric-strip article { min-height: 96px; padding: 14px 18px 16px; border-right: 1px solid #28344a; }.metric-strip article:last-child { border-right: 0; }.metric-strip span { display: block; color: #aab5c7; font-size: 18px; line-height: 1.1; }.metric-strip strong { display: block; margin-top: 5px; color: #e8edf6; font: 600 32px var(--font-family-mono, monospace); line-height: 1.05; }.metric-strip small { margin-left: 4px; color: #aab5c7; font-size: 16px; font-weight: 500; }.metric-strip p { margin: 5px 0 0; color: #8492a8; font-size: 14px; line-height: 1.2; }.metric-strip .tone-success strong { color: #72deb9; }.metric-strip .tone-info strong { color: #85aefd; }.metric-strip .tone-warning strong { color: #edc66b; }.metric-strip .tone-danger strong { color: #f17a89; }
