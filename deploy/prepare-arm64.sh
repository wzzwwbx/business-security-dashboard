...
rm -rf "$OUT_DIR/frontend" "$OUT_DIR/database" "$OUT_DIR/deploy"
mkdir -p "$OUT_DIR/frontend" "$OUT_DIR/database" "$OUT_DIR/deploy"
...
cp "$ROOT_DIR/deploy/compose.offline.yml" "$ROOT_DIR/deploy/nginx.conf" \
  "$ROOT_DIR/deploy/install-offline.sh" "$OUT_DIR/deploy/"
